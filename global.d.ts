...
declare module 'accept-language-parser' {
     ...
     * @param {any} options `options` 매개변수는 다음 속성을 포함할 수 있는 선택적 개체입니다.
     * @param {boolean} options.loose - 지역과 스크립트가 일치하는 경우에만 `language-code`가 일치하는 지원되는 언어를 선택하거나 false로 설정합니다. 기본값은 false입니다.
     * @returns {any} - 수락 언어와 일치하는 지원되는 언어 또는 일치하는 항목이 없으면 null
     */
